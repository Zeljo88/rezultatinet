            ...
            <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3">
                ...
                <a href="/igraci/{{ $player->slug }}" class="bg-[#1a1a1a] border border-[#2a2a2a] hover:border-[#CCFF00] rounded-xl p-4 text-center transition cursor-pointer group">
                    <div class="relative mb-3">
                        ...
                        <div class="w-16 h-16 rounded-full overflow-hidden bg-[#2a2a2a] flex items-center justify-center mx-auto">
                            @if($player->photo_url)
                                <img src="{{ $player->photo_url }}"
                                     alt="{{ $player->name }}"
                                     class="w-full h-full object-cover"
                                     onerror="this.style.display='none'; this.nextElementSibling.style.display='flex';">
                                <div style="display:none" class="w-full h-full flex items-center justify-center text-2xl font-black text-[#CCFF00]">
                                    {{ strtoupper(substr($player->name, 0, 1)) }}
                                </div>
                            @else
                                <div class="w-full h-full flex items-center justify-center text-2xl font-black text-[#CCFF00]">
                                    {{ strtoupper(substr($player->name, 0, 1)) }}
                                </div>
                            @endif
                        </div>
        ...
        <div class="border border-[#2a2a2a] rounded-xl overflow-hidden">
            ...
            <a href="/igraci/{{ $player->slug }}"
               class="flex items-center gap-3 px-4 py-3 hover:bg-[#222] transition border-b border-[#2a2a2a] last:border-0 cursor-pointer {{ $i % 2 === 0 ? 'bg-[#0f0f0f]' : 'bg-[#161616]' }}">
                <div class="w-9 h-9 rounded-full overflow-hidden bg-[#2a2a2a] flex items-center justify-center flex-shrink-0">
                    @if($player->photo_url)
                        <img src="{{ $player->photo_url }}"
                             alt="{{ $player->name }}"
                             class="w-full h-full object-cover"
                             onerror="this.style.display='none'; this.nextElementSibling.style.display='flex';">
                        <div style="display:none" class="w-full h-full flex items-center justify-center text-sm font-black text-[#CCFF00]">
                            {{ strtoupper(substr($player->name, 0, 1)) }}
                        </div>
                    @else
                        <div class="w-full h-full flex items-center justify-center text-sm font-black text-[#CCFF00]">
                            {{ strtoupper(substr($player->name, 0, 1)) }}
                        </div>
                    @endif
                </div>
