<div class="bg-[#1a1a2e] border border-white/5 rounded-xl p-5 mb-4">
    <h3 class="text-sm font-bold uppercase tracking-wider mb-4" style="color:#CCFF00">Ko će pobijediti?</h3>

    @if(! && !)
        {{-- Voting buttons (pre-vote, match not finished) --}}
    ...
    @elseif( && ['total'] > 0)
        {{-- POST-MATCH: show results + who was right --}}
        @php
            $winningPct = $stats[$actualResult];
            $userWon = $hasVoted && $userVote === $actualResult;
            $labels = ['home' => $homeTeam, 'draw' => 'Remi', 'away' => $awayTeam];
            $maxPct = max($stats['home'], $stats['draw'], $stats['away']);
        @endphp
        ...
        <div class="mb-4 rounded-lg p-3 text-center {{ $userWon ? 'bg-[#CCFF00]/10 border border-[#CCFF00]/30' : 'bg-white/5 border border-white/10' }}">
            @if($hasVoted)
                ...
                @endif
            @endif
            <div class="text-white font-bold text-lg">{{ $winningPct }}% navijača je pogodilo!</div>
        ...
        <div class="space-y-3"
            ...
            @foreach(['home' => $homeTeam, 'draw' => 'Remi', 'away' => $awayTeam] as $key => $label)
                @php
                    $isWinner = $key === $actualResult;
                    $isUserVote = $hasVoted && $userVote === $key;
                @endphp
                <div>
                    <div class="flex justify-between text-xs mb-1">
                        <span class="{{ $isWinner ? 'text-[#CCFF00] font-bold' : 'text-gray-300' }} truncate max-w-[140px]">
                            {{ $label }}
                            @if($isWinner) <span class="ml-1">✓</span> @endif
                            @if($isUserVote && !$isWinner) <span class="text-gray-500 ml-1">(tvoj glas)</span> @endif
                            @if($isUserVote && $isWinner) <span class="text-[#CCFF00] ml-1">(tvoj glas)</span> @endif
                        </span>
                        <span class="font-bold {{ $isWinner ? 'text-[#CCFF00]' : 'text-gray-400' }}">{{ $stats[$key] }}%</span>
                    </div>
                    <div class="h-2 bg-white/10 rounded-full overflow-hidden">
                        <div
                            class="h-full rounded-full transition-all duration-700 ease-out {{ $isWinner ? 'bg-[#CCFF00]' : 'bg-white/20' }}"
                            x-bind:style="show ? 'width: {{ $stats[$key] }}%' : 'width: 0%'"
                        ></div>
                    </div>
                </div>
            @endforeach
    ...
    @else
        {{-- Voted, match still in progress — show live bars --}}
            ...
            @foreach(['home' => $homeTeam, 'draw' => 'Remi', 'away' => $awayTeam] as $key => $label)
                <div>
                    <div class="flex justify-between text-xs mb-1">
                        ...
                        <span class="font-bold {{ $stats[$key] === $maxPct && $maxPct > 0 ? 'text-[#CCFF00]' : 'text-gray-300' }}">{{ $stats[$key] }}%</span>
                    </div>
                    <div class="h-2 bg-white/10 rounded-full overflow-hidden">
                        <div
                            class="h-full rounded-full transition-all duration-700 ease-out {{ $stats[$key] === $maxPct && $maxPct > 0 ? 'bg-[#CCFF00]' : 'bg-white/30' }}"
                            x-bind:style="show ? 'width: {{ $stats[$key] }}%' : 'width: 0%'"
                        ></div>
                    </div>
                </div>
